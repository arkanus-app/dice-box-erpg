import { aS as yn, L as Sn, $ as G } from "./Dice-f63fdd4d.js";
const ln = 4, X = 4, an = 1, C = 2, xn = 8, H = 65536, j = H >> 3, En = 16, T = 14, L = (1 << En) + 1, V = 1 << T, d = V - 1, K = 59, sn = 63, vn = 2 + sn - K;
var U;
(function(n) {
...
function Rn(n, t, e) {
  const l = t.value, r = [];
  for (; t.value < l + e - 1; ) {
    const c = q(n.buffer, t), s = P(n, t), o = Z(n, t);
    t.value += 3;
...
function zn(n, t) {
  ...
  let s = !0;
  for (; s; ) {
    const o = q(n.buffer, t);
    if (!o)
      s = !1;
    else {
      const i = q(n.buffer, t), a = m(n, t), u = Fn(n, t, i, a);
...
function Xn(n, t, e, l, r, c, s, o, i) {
  let a = 0, u = 0;
  const f = s, E = Math.trunc(l.value + (r + 7) / 8);
  for (; l.value < E; ) {
    let p = J(a, u, e, l);
...
function $n(n, t, e, l, r, c, s) {
  ...
  for (a >>= 1, u = a, a >>= 1; a >= 1; ) {
    f = 0;
    const E = f + c * (r - u), I = c * a, p = c * u, w = l * a, y = l * u;
    let h, v, A, F;
    for (; f <= E; f += p) {
      let x = f;
      const W = f + l * (e - u);
      for (; x <= W; x += y) {
        const O = x + w, g = x + I, k = g + w;
        if (o) {
          let S = N(n[x + t], n[g + t]);
          h = S.a, A = S.b, S = N(n[O + t], n[k + t]), v = S.a, F = S.b, S = N(h, v), n[x + t] = S.a, n[O + t] = S.b, S = N(A, F), n[g + t] = S.a, n[k + t] = S.b;
        } else {
          let S = R(n[x + t], n[g + t]);
          h = S.a, A = S.b, S = R(n[O + t], n[k + t]), v = S.a, F = S.b, S = R(h, v), n[x + t] = S.a, n[O + t] = S.b, S = R(A, F), n[g + t] = S.a, n[k + t] = S.b;
        }
      }
      if (e & a) {
        const O = x + I;
        let g;
        o ? g = N(n[x + t], n[O + t]) : g = R(n[x + t], n[O + t]), h = g.a, n[O + t] = g.b, n[x + t] = h;
      }
    }
    if (r & a) {
      let x = f;
...
function jn(n) {
  ...
  for (let p = 0; p < n.channels; ++p) {
    ...
    for (let y = 0; y < s[p].size; ++y)
      $n(l, w.start + y, w.nx, w.size, w.ny, w.nx * w.size, u);
  }
  qn(a, l, c);
  let E = 0;
  ...
  for (let p = 0; p < n.lines; p++)
    for (let w = 0; w < n.channels; w++) {
      const y = s[w], h = y.nx * y.size, v = new Uint8Array(l.buffer, y.end * C, h * C);
      I.set(v, E), E += h * C, y.end += h;
    }
...
async function dn(n, t, e, l) {
  ...
  if (r.type === 1)
    switch (l) {
      case _.Float:
        r.getter = In, r.inputSize = C;
        break;
      case _.HalfFloat:
        r.getter = B, r.inputSize = C;
        break;
    }
  else if (r.type === 2)
    switch (l) {
